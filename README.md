...
| `ecs.entity_new(w)` / `ecs.destroy_entity(w, e)`       | сущности (id переиспользуются)              |
| `ecs.add_component(w, e, value)`                       | добавить/записать компонент                 |
| `ecs.get_component(w, e, T) -> ^T`                     | прочитать/изменить (nil — нет)              |
| `ecs.remove_component(w, e, T)`                        | удалить компонент                           |
...
| `ecs.cmds_destroy(ctx.cmds, e)`                        | отложенное уничтожение (безопасно в update) |

Важные детали:

- **Отложенное уничтожение**: `cmds_destroy` копит сущности, `cmds_flush` (в `run`, после всех
  update-систем) применяет их. Так можно уничтожать сущности прямо во время итерации запроса
  (см. `bullet_collision` в примере). Дубли в очереди исключаются.
